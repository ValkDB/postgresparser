...
| `CREATE TABLE` | `DDL` | `Tables`, `DDLActions` (with `ColumnDetails`, `PrimaryKey`, `ForeignKeys`, `UniqueKeys`, `CheckConstraints`) |
| `ALTER TABLE` | `DDL` | `Tables`, `DDLActions` (including PK/FK/UNIQUE/CHECK metadata on `ADD CONSTRAINT`) |
