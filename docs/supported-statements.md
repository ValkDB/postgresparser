...
| `CREATE TABLE` | `DDL` | `Tables`, `DDLActions` (with `ColumnDetails`, `PrimaryKey`, `ForeignKeys`, `UniqueKeys`) |
| `ALTER TABLE` | `DDL` | `Tables`, `DDLActions` (including PK/FK/UNIQUE metadata on `ADD CONSTRAINT`) |
